...
def test_s2u(comm, subset_output_loc, write_output):
  ...
  if write_output:
    ...
    MIO.dist_tree_to_file(dist_tree, os.path.join(out_dir, 'tree_U.hdf'), comm)
...
def test_s2u_withdata(comm, write_output):
  ...
  bc_right = PT.yaml.to_node(
    ...
    """)
  PT.rm_nodes_from_name(dist_tree, 'Right')
  PT.add_child(PT.get_node_from_label(dist_tree, 'ZoneBC_t'), \
      MF.full_to_dist.distribute_pl_node(bc_right, comm))

  convert_s_to_ngon(dist_tree, comm)

  # Some checks
  bc_right_u = PT.get_node_from_name(dist_tree, 'Right')
