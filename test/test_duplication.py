...
def test_duplicate_from_periodic(sub_comm, write_output):
  ...
  DUP.duplicate_n_zones_from_periodic_join(dist_tree, [dist_zone], opposite_jns, duplication_number=4)
...
def test_duplicate_360(sub_comm, write_output):
  ...
  assert len(I.getZones(dist_tree)) == 1
  DUP.duplicate_zones_from_periodic_join_by_rotation_to_360(dist_tree, I.getZones(dist_tree), opposite_jns, comm=sub_comm)
