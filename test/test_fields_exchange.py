...
class Test_fields_exchange:
  ...
  def test_created_sol(self, sub_comm):
    ...
    for part_zone in part_zones:
      ...
      I.newDataArray('PartRankId', sub_comm.Get_rank() * np.ones(PT.Zone.n_cell(part_zone)), part_fs)
      part_fs_new = I.newFlowSolution('CreatedFS', gridLocation='Vertex', parent=part_zone)
      I.newDataArray('PartRankId', sub_comm.Get_rank() * np.ones(PT.Zone.n_vtx(part_zone)), part_fs_new)
