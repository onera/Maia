...
def test_spherical_slice_U(elt_type,sub_comm, write_output):
  ...
  for zone in PT.get_all_Zone_t(part_tree):
    ...
    cell_center = maia.algo.part.geometry.compute_cell_center(pzone)
    ccx = cell_center[0::3]
    ccy = cell_center[1::3]
    ccz = cell_center[2::3]

    # Fields
    fld1_nc =   cx**2 +  cy**2 +  cz**2 - 1
    fld2_nc =   cx**2 +  cy**2          - 1
    fld1_cc =  ccx**2 + ccy**2 + ccz**2 - 1
