...
from maia.connectivity import conformize_jn as CJN

@mark_mpi_test(2)
@pytest.mark.parametrize("from_loc", ["Vertex", "FaceCenter"])
def test_conformize_jn(sub_comm, from_loc):
  ...
  """
  gcs = [DN.distribute_pl_node(gc, sub_comm) for gc in parse_yaml_cgns.to_nodes(yt)]
  zone = I.getZones(dist_tree)[0]
  I.createChild(zone, "ZGC", "ZoneGridConnectivity_t", children=gcs)

  CJN.conformize_jn(dist_tree, ['Base/zone/ZGC/matchA', 'Base/zone/ZGC/matchB'], sub_comm)
