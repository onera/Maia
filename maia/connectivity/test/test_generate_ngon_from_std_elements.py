...
class Test_compute_ngon_from_std_elements:
  ...
  def test_3d_mesh(self, sub_comm):
    ...
    if rank == 0:
      assert (I.getVal(IE.getDistribution(ngon, 'Element')) == [0,27,56]).all()
  ...
  def test_2d_mesh(self, sub_comm):
    ...
    if rank == 0:
      assert (I.getVal(IE.getDistribution(ngon, 'Element')) == [0,9,17]).all()
      ...
      assert (I.getVal(IE.getDistribution(ngon, 'Element')) == [9,17,17]).all()
