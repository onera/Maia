...
def create_subset_numbering(subset_l, parent_numbering_l, comm):
  ...
  for i_part in range(n_part):
    extracted_ln_to_gn = EX.extract_from_indices(parent_numbering_l[i_part], subset_l[i_part], 1, 1)
  ...
  if strategy == 'Closest' or (strategy == 'LocationAndClosest' and n_tot_unlocated > 0):
    ...
    if strategy != 'Closest':
      for i_domain, part_zones in enumerate(tgt_parts_per_dom):
        ...
        unlocated_gnum["extracted_coords"] = [EX.extract_from_indices(all_tgt_coords[i_domain][i_part], \
