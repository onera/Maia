...
def test_localize_mdom(comm):
  ...
  pts_inside_cnt = np.diff(result_inv_large['elt_pts_inside_idx'])
  expected = np.zeros_like(pts_inside_cnt)
  if comm.rank == 0:
    assert np.array_equal(pts_inside_cnt, expected)
    assert result_inv_large['points_gnum'].size == 0
  if comm.rank == 1:
    expected[83] = 1
    assert np.array_equal(pts_inside_cnt, expected)
    assert np.array_equal(result_inv_large['points_gnum'], [3])

  # Point in small zone is in cell 8, which in on rank 0 (local id = 8 - 1 = 7)
  pts_inside_cnt = np.diff(result_inv_small['elt_pts_inside_idx'])
  expected = np.zeros_like(pts_inside_cnt)
  if comm.rank == 0:
    expected[7] = 1
    assert np.array_equal(pts_inside_cnt, expected)
    assert np.array_equal(result_inv_small['points_gnum'], [2])
  if comm.rank == 1:
    assert np.array_equal(pts_inside_cnt, expected)
    assert result_inv_small['points_gnum'].size == 0