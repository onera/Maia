...
def test_suffix_bases():
  ...
  """)
  MG._suffix_bases(tree, '.MG8')

  assert [PT.get_name(b) for b in PT.get_all_CGNSBase_t(tree)] == \
    ['SomeBase.MG8', 'SomeOtherBase.MG8']
  assert PT.get_value(PT.find_node_from_name(tree, 'jn')) == "SomeBase.MG8/Zone"
...
def test_multigrid_s_2D(comm):
  ...
  MG.multigrid_s(tree, n_lvl, comm)
  ...
  for i in range(n_lvl):
    ...
    PT.keep_children_from_name(thin, f'Base.MG{i}')
    coarse = PT.shallow_copy(tree)
    PT.keep_children_from_name(coarse, f'Base.MG{i+1}')
    ...
    expected_i = PT.get_np_value(PT.find_node_from_path(thin, f'Base.MG{i}/zone/CoarseId/I'))
    computed_i = PT.get_np_value(PT.find_node_from_path(thin, f'Base.MG{i}/zone/MultiGridCellInfo/ICoarseIdx'))
    assert np.array_equal(computed_i, expected_i)

    expected_j = PT.get_np_value(PT.find_node_from_path(thin, f'Base.MG{i}/zone/CoarseId/J'))
    computed_j = PT.get_np_value(PT.find_node_from_path(thin, f'Base.MG{i}/zone/MultiGridCellInfo/JCoarseIdx'))
...
def test_multigrid_s(comm):
  ...
  MG.multigrid_s(dt, nb_lvl, comm)
  
  assert(len(PT.get_all_CGNSBase_t(dt)) == nb_lvl+1)
  
  for t in range(nb_lvl+1):
    zone = PT.find_node_from_path(dt, f'Base.MG{t}/zone')
  ...
  icoarse_ids_lvl0 = PT.get_value(PT.get_node_from_path(dt, 'Base.MG0/zone/MultiGridCellInfo/ICoarseIdx'))
  ...
  jcoarse_ids_lvl0 = PT.get_value(PT.get_node_from_path(dt, 'Base.MG0/zone/MultiGridCellInfo/JCoarseIdx'))
  ...
  kcoarse_ids_lvl0 = PT.get_value(PT.get_node_from_path(dt, 'Base.MG0/zone/MultiGridCellInfo/KCoarseIdx'))
  ...
  icoarse_ids_lvl1 = PT.get_value(PT.get_node_from_path(dt, 'Base.MG1/zone/MultiGridCellInfo/ICoarseIdx'))
  assert np.all(icoarse_ids_lvl1==1)
  jcoarse_ids_lvl1 = PT.get_value(PT.get_node_from_path(dt, 'Base.MG1/zone/MultiGridCellInfo/JCoarseIdx'))
  assert np.all(jcoarse_ids_lvl1==1)
  kcoarse_ids_lvl1 = PT.get_value(PT.get_node_from_path(dt, 'Base.MG1/zone/MultiGridCellInfo/KCoarseIdx'))
  assert np.all(kcoarse_ids_lvl1==1)
  
  assert PT.get_node_from_path(dt, 'Base.MG2/zone/MultiGridCellInfo') == None
