...
class Test_compute_ngon_from_std_elements:
  ...
  def test_3d_mesh(self, comm):
    ...
    MT.new_Distribution({'Vertex' : [9*rank,9*(rank+1),18], 'Cell' : [10*rank, 10*(rank+1), 20]}, dist_zone)
    tetra = PT.new_Elements('Tetra', 'TETRA_4', erange=[1,20], econn=tetra_ec, parent=dist_zone)
    MT.new_Distribution({'Element' : np.array([10*rank,10*(rank+1),20], pdm_dtype)}, tetra)

    GNG.generate_ngon_from_std_elements(dist_tree, comm)

    assert PT.get_node_from_path(dist_tree, 'Base/Zone/Tetra') is None
    ngon  = PT.find_node_from_path(dist_tree, 'Base/Zone/NGonElements')
    nface = PT.find_node_from_path(dist_tree, 'Base/Zone/NFaceElements')
    ...
    if rank == 0:
      ...
      assert (MT.find_Distribution(ngon, 'ElementConnectivity')[1] == [0,81,168]).all()
      ...
      assert (MT.find_Distribution(ngon, 'ElementConnectivity')[1] == [81,168,168]).all()
  ...
  def test_2d_mesh(self, comm):
    ...
    MT.new_Distribution({'Vertex' : np.array([6*rank,6*(rank+1),12], pdm_dtype),
    ...
    MT.new_Distribution({'Element' : np.array([3*rank,3*(rank+1),6], pdm_dtype)}, quad)

    GNG.generate_ngon_from_std_elements(dist_tree, comm)

    assert PT.get_node_from_path(dist_tree, 'Base/Zone/Quad') is None
    edge  = PT.find_node_from_path(dist_tree, 'Base/Zone/EdgeElements')
    ngon  = PT.find_node_from_path(dist_tree, 'Base/Zone/NGonElements')
    ...
    if rank == 0:
      ...
      assert (MT.find_Distribution(ngon, 'ElementConnectivity')[1] == [0,12,24]).all()
      ...
      assert (MT.find_Distribution(ngon, 'ElementConnectivity')[1] == [12,24,24]).all()

      assert (PT.find_child_from_name(edge, 'ElementConnectivity')[1] == [9,5,8,7,6,10,7,11,10,9,8,12,11,10,12,11]).all()
      assert (PT.find_child_from_name(ngon, 'ElementStartOffset')[1] == [12,16,20,24]).all()
      assert (PT.find_child_from_name(ngon, 'ElementConnectivity')[1] == [5,6,10,9, 10,6,7,11, 11,7,8,12]).all()
  ...
  def test_2d_mesh_with_subsets(self, comm):
    ...
    if rank == 0:
      ...
      assert (PT.get_child_from_name(bcd, 'PointRange')[1] == [20,23]).all()
