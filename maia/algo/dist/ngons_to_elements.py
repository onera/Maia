...
def _ngon_to_elements_zone_3d(zone, comm):
  ...
  cell_nvtx_per_face = EP.block_to_part(face_n, face_distri, np.abs(cell_face.values)-1, comm, legacy=False)
  ...
  sections_stride, sections_face_vtx = EP.block_to_part_strided(face_n, face_vtx.values, face_distri, [np.abs(p)-1 for p in cell_face_section], comm, legacy=False)
