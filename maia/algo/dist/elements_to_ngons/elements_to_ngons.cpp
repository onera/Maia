...
concatenate_into_poly_section(const cgns::tree_range& section_nodes, ElementType_t poly_section_type, MPI_Comm comm) {
  ...
  std::vector<std_e::interval_vector<PDM_g_num_t>> connec_distri_by_section(n_section);
  ...
  for (int i=0; i<n_section; ++i) {
    ...
    auto elt_distri = ElementDistribution<PDM_g_num_t>(elt_node);
...
ngon_from_faces(const tree_range& face_sections, MPI_Comm comm) -> tree {
  ...
  std::vector<std_e::interval_vector<PDM_g_num_t>> distri_by_section(n_section);
  for (int i=0; i<n_section; ++i) {
...
auto
elements_to_ngons(tree& z, MPI_Comm comm) -> void {
  STD_E_ASSERT(is_maia_compliant_zone(z));
  auto _ = maia_perf_log_lvl_0("elements_to_ngons");
