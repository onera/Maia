...
def minimal_partitioning(zone, comm, use_geom=False):
  ...
  if is_poly_3d_zone(zone):
    ...
    if not use_geom and comm.allreduce(np.array_equal(cell_distri, nface_distri), MPI.LAND):
      ...
      pcell_face_n, pcell_face_val = EP.block_to_part_strided(dcell_face.counts, dcell_face.values, nface_distri, cell_gnum-1, comm)
    ...
    pface_vtx_n, pface_vtx = EP.block_to_part_strided(dface_vtx.counts, dface_vtx.values, ngon_distri, face_gnum-1, comm)
    ...
    if not use_geom:
      ...
      pface_edge_n, pface_edge_val = EP.block_to_part_strided(dface_edge.counts, dface_edge.values, _face_distri, cell_gnum-1, comm)
    ...
    if not use_geom:
      ...
      pcell_vtx_n, pcell_vtx_val = EP.block_to_part_strided(dcell_vtx.counts, dcell_vtx.values, cell_distri, cell_gnum-1, comm)
