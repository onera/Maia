...
def duplicate_elts(zone, elt_pl, elt_name, as_bc=None, elts_to_update=dict(), periodic=None, elt_duplicate_bcs=dict()):
  is_asked_elt = lambda n: PT.get_label(n)=='Elements_t' and\
...
def duplicate_periodic_patch(tree, gc_paths, periodic_values):
  ...
  for i_per in range(n_periodicity):
    ...
    new_vtx_num = duplicate_elts(zone, face_pl, 'TRI_3', f'tri_3_periodic_{i_per}', elts_to_update, periodic_values[1][i_per])
    to_constrain_bcs.append(face_bc_name)
...
def retrieve_initial_domain(tree, gc_paths, periodic_values, new_vtx_num,
  ...
  for i_per in range(n_periodicity-1, -1, -1):
    ...
    _ = duplicate_elts(zone, face_pl, 'TRI_3', to_retrieve_gc_name, elts_to_update, periodic_values[0][i_per], elt_duplicate_bcs=bcs_to_retrieve[i_per])
