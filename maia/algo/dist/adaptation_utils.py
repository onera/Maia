...
IS_LOC_FS = PTp.label_in(['FlowSolution_t', 'DiscreteData_t']) & PTp.has_location('Vertex')
...
def duplicate_specified_vtx(zone, vtx_pl, comm):
  ...
  # Update GridCoordinates
  GI = EP.GlobalIndexer(new_distri, vtx_gnum, comm)
...
def remove_specified_vtx(zone, vtx_pl, comm):
  ...
  # Get ids to remove
  GI = EP.GlobalIndexer(distri, vtx_pl-1, comm)
...
def elmt_pl_to_vtx_pl(zone, elt_n, elt_pl, comm):
  ...
  GI = EP.GlobalIndexer(vtx_distri, pl_ec-1, comm)
...
def find_shared_faces(tri_elt, tri_pl, tetra_elt, tetra_pl, comm):
  ...
  GI = EP.GlobalIndexer(src_distri, tri_pl - PT.Element.Range(tri_elt)[0], comm)
  ...
  GI = EP.GlobalIndexer(tgt_distri, tetra_pl - PT.Element.Range(tetra_elt)[0], comm)
  ...
  GI = EP.GlobalIndexer(distri, [tri_key-1, tetra_key-1], comm)
...
def merge_periodic_bc(zone, bc_names, vtx_tag, old_to_new_vtx_num, comm, keep_original=False):
  ...
  GI = EP.GlobalIndexer(vtx_distri, pbc1_vtx_pl-1, comm)
  pl1_tag = vtx_tag[GI.access_counts > 0]

  GI = EP.GlobalIndexer(vtx_distri, pbc2_vtx_pl-1, comm)
...
def duplicate_elts(zone, elt_n, elt_pl, as_bc, elts_to_update, comm, elt_duplicate_bcs=[]):
  ...
  EP.GlobalIndexer(vtx_distri, elt_vtx_pl-1, comm).Put(new_vtx_pl, old_to_new_vtx)
  ...
  GI = EP.GlobalIndexer(elt_distri, elt_pl_shft, comm)
  ids = np.flatnonzero(GI.access_counts > 0)
  ...
  GI = EP.GlobalIndexer(new_elt_distri, elt_gnum, comm)
  ...
  if elt_duplicate_bcs != []:
    ...
    for matching_bcs in elt_duplicate_bcs:
      ...
      GI = EP.GlobalIndexer(elt_distri, bc_pl_shft, comm)
...
def find_matching_bcs(zone, elt_n, src_pl, tgt_pl, src_tgt_vtx, comm):
  ...
  GI = EP.GlobalIndexer(vtx_distri, src_tgt_vtx[0]-1, comm)
  ...
  for i_side, _bc_nodes in enumerate(bc_nodes):
    for src_bc_n in _bc_nodes:
      ...
      # > Get BC connectivity
      GI = EP.GlobalIndexer(elt_distri, bc_pl-elt_offset, comm)
      ids = np.flatnonzero(GI.access_counts > 0)
      ...
      GI = EP.GlobalIndexer(vtx_distri, this_bc_vtx-1, comm)
...
def add_undefined_faces(zone, elt_n, elt_pl, tgt_elt_n, comm, bc_names=list()):
  ...
  for bc_name in bc_names:
    ...
    GI = EP.GlobalIndexer(tgt_elt_distri, bc_pl-tgt_elt_offset, comm)
    ids = np.flatnonzero(GI.access_counts > 0)
  ...
  for gc_paths, periodic_values in reversed(list(jn_pairs_and_values.items())): # reversed for future multiperiodic (use list() for py 3.7 support)
    ...
    if PT.get_node_from_name_and_label(zone, to_retrieve_gc_name+'_c', 'BC_t') is not None:
      ...
      for bc_names in src_bc_names:
        wanted_bc = PTp.label_is('BC_t') & PTp.name_in(bc_names)
