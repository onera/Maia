...
def compute_edge_center(zone, comm, edge_indices=None):
  ...
  part_data = EP.block_to_part(dist_coords, vtx_distri, edge_vtx.values-1, comm, legacy=False)
...
def compute_face_center(zone, comm, face_indices=None, face_indices_loc=None):
  ...
  # Other cases (Structured 3D or NGON) does not manage idx filtering : we do it manually
    ...
    if face_indices is not None:
      ...
      face_vtx_n, face_vtx_v = EP.block_to_part_strided(face_vtx.counts, face_vtx.values, face_distri, _face_indices, comm, legacy=False)
  ...
  part_data = EP.block_to_part(dist_coords, vtx_distri, face_vtx.values-1, comm, legacy=False)
  local_coords = [part_data[key] for key in part_data.keys()]
...
def compute_cell_center(zone, comm, cell_indices=None):
  ...
  part_data = EP.block_to_part(dist_coords, vtx_distri, cell_vtx.values-1, comm, legacy=False)
  local_coords = [part_data[key] for key in part_data.keys()]
