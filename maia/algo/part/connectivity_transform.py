...
def enforce_boundary_pe_left(zone_node: CGNSTree) -> None:
  ...
  if z_dim == 3:
    try:
      ...
      enforce_pe_left_parent(PT.find_child_from_name(ngon, 'ElementStartOffset')[1],
                             PT.find_child_from_name(ngon, 'ElementConnectivity')[1],
                             PT.find_child_from_name(ngon, 'ParentElements')[1],
                             PT.find_child_from_name(nface, 'ElementStartOffset')[1],
                             PT.find_child_from_name(nface, 'ElementConnectivity')[1])
    ...
    if len(bar_elts) > 1:
      ...
      enforce_pe_left_parent(nedge_eso,
                             PT.find_child_from_name(nedge, 'ElementConnectivity')[1],
                             PT.find_child_from_name(nedge, 'ParentElements')[1])
