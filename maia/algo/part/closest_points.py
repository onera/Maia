...
def _closest_points(src_clouds: List[PointCloud],
                    ...
                    n_pts: int, 
                    need_shift:bool) -> Tuple[List[Result], List[InvResult]]: ...
...
def _closest_points(src_clouds: List[PointCloud],
                    ...
                    reverse: bool,
                    n_pts: int = 1, 
                    need_shift:bool = False) -> Union[List[Result],
  ...
  If target lngn does is not a contiguous range 1...n_tgt_pts, need_shift must be True to allow
  the creation of a (temporary) subnumbering
  """
  ...
  # > Setup target
  if need_shift:
    ini_tgt_gnum = [tgt_cloud[1] for tgt_cloud in tgt_clouds]
    sub_tgt_gnum = create_sub_numbering(ini_tgt_gnum, comm)
    tgt_clouds = [(tgt_cloud[0], sub_lngn) for tgt_cloud, sub_lngn in zip(tgt_clouds, sub_tgt_gnum)]
  ...
  if reverse:
    _results = [closest_point.tgt_in_src_get(i_src_part) for i_src_part in range(len(src_clouds))]
    all_closest_inv = []
    # Retrieve target original gnum (transform inplace)
    if need_shift:
      gnum_to_transform = [_result["tgt_in_src"] for _result in _results]
      PDM.transform_to_parent_gnum(gnum_to_transform, sub_tgt_gnum, ini_tgt_gnum, comm)
    for i_src_part in range(len(src_clouds)):
      _result = _results[i_src_part]
...
def _mdom_closest_points(src_clouds_per_dom:List[List[PointCloud]], 
                         ...
                         comm:MPIComm,
                         reverse:Literal[False],
                         **kwargs) -> List[List[Result]]: ...
@overload
def _mdom_closest_points(src_clouds_per_dom:List[List[PointCloud]], 
                         tgt_clouds_per_dom:List[List[PointCloud]],
                         comm:MPIComm,
                         reverse:Literal[True],
                         **kwargs) -> Tuple[List[List[Result]], List[List[InvResult]]]: ...
...
def _mdom_closest_points(src_clouds_per_dom:List[List[PointCloud]], 
  ...
  if reverse:
    direct_result, inv_result = _closest_points(src_clouds, tgt_clouds, comm, True, **kwargs)
  else:
    direct_result = _closest_points(src_clouds, tgt_clouds, comm, False, **kwargs)
...
def _find_closest_points(src_parts_per_dom: List[List[CGNSPartTree]], 
  ...
  return _mdom_closest_points(src_clouds, tgt_clouds, comm, reverse, **kwargs)
...
def find_closest_points(src_tree: CGNSPartTree, 
  ...
  closest_data = _find_closest_points(src_parts_per_dom, tgt_parts_per_dom, location, location, comm, False)
