...
from maia.algo.part import part_to_part_utils as PTPU
...
def test_get_relative_pl():
  ...
  pl1_n = PT.get_value(PTPU.get_relative_pl(zsr1, part_zone))[0]
  pl2_n = PT.get_value(PTPU.get_relative_pl(zsr2, part_zone))[0]
  pl3_n = PT.get_value(PTPU.get_relative_pl(zsr3, part_zone))[0]
...
def test_local_pl_offset():
  ...
  assert PTPU.local_pl_offset(zone, 0) == 0
  assert PTPU.local_pl_offset(zone, 2) == 0
  assert PTPU.local_pl_offset(zone, 3) == 10
  zone = PT.new_Zone('Zone', type='Unstructured')
  PT.new_NFaceElements('NFace', erange=[1, 5], parent=zone)
  PT.new_NGonElements('NGon', erange=[6, 10], parent=zone)
  assert PTPU.local_pl_offset(zone, 2) == 5
  assert PTPU.local_pl_offset(zone, 3) == 0
  zone = PT.new_Zone('Zone', type='Unstructured')
  PT.new_Elements('TRI_3', type='TRI_3',erange=[1, 5], parent=zone)
  PT.new_Elements('BAR_2', type='BAR_2',erange=[6,15], parent=zone)
  assert PTPU.local_pl_offset(zone, 0) == 0
  assert PTPU.local_pl_offset(zone, 1) == 5
  assert PTPU.local_pl_offset(zone, 2) == 0
...
def test_get_partial_container_stride_and_order(sub_comm):
  ...
  PROC 0:
    ...
    part1:
      ...
      ref_lnum2      = [1 2]
        --> first two elts of part2 on proc0
            are linked with [2 8] of part1 
            (proc1: p1_to_p2 = [x,x,6,7])
      PL_part2 = []
        --> pl_gnum1=[] 
    ...
    part0:
      ...
      --> elts [1 2 5] of part2 on proc1
          ...
          (proc0: p1_to_p2 = [9,10,9,10,13]
           ...
           part_to_part)
      PL_part2 = [4 2], elts 10 only is a parent of two elt of part1
  ...
  # Container 
  pl_gnum1, stride = PTPU.get_partial_container_stride_and_order(part_zones, 'FSol_A', 'Vertex', ptp, sub_comm)
  if sub_comm.Get_rank()==0:
