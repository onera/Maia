...
def test_get_part_data(elt_kind, comm):
  ...
  if elt_kind == 'Poly':
    ...
    assert len(data) == 8
    assert (data[2] == MT.Zone.cell_globalnumbering(zone)).all()
    assert (data[7] == MT.Zone.vtx_globalnumbering(zone)).all()
    assert data[6].size == 3*PT.Zone.n_vtx(zone) #Coords
  elif elt_kind == 'HEXA_8':
    data = LOC._get_part_data_elts(zone)
    assert len(data) == 5
    assert (data[2] == MT.Zone.cell_globalnumbering(zone)).all()
    assert (data[4] == MT.Zone.vtx_globalnumbering(zone)).all()
...
def test_mesh_location(reverse, comm):
  ...
  if reverse:
    ...
    for i_part, zone in enumerate(PT.get_all_Zone_t(tree)):
      cell_gnum = MT.Zone.cell_globalnumbering(zone).astype(np.int64)
...
def test_localize_points(input_kind, comm):
  ...
  assert loc_node is not None and PT.Container.GridLocation(loc_node) == 'CellCenter'
