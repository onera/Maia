...
def test_get_shifted_arrays(comm):
  if comm.Get_rank() == 0:
  ...
  offset, shifted_arrays = MGM._get_shifted_arrays(arrays, comm)
...
def test_get_mdom_gnum_vtx(unify_jns, comm):
  if comm.Get_rank() == 0:
    yt = """
    ZoneA.P0.N0 Zone_t:
  ...
  global_gnum = MGM.get_mdom_gnum_vtx(part_per_doms, comm, unify_jns)
