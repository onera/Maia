...
class Test_cell_vtx_connectivity:
  ...
  def test_ngon3d(self, elt_kind, comm):
    ...
    as_partitioned(zone)
    cell_vtx = CU.cell_vtx_connectivity(zone)
    ...
    assert len(cell_vtx) == 2**3
    assert (cell_vtx.counts == 8).all()
    assert (cell_vtx.values == expected_cell_vtx).all()
  ...
  def test_elts(self, elt_kind, comm):
    ...
    cell_vtx = CU.cell_vtx_connectivity(zone, dim_zone)

    elt = PT.get_node_from_label(zone, 'Elements_t')
    assert len(cell_vtx) == PT.Element.Size(elt)
