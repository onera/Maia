...
def test_simple():
  ...
  assert 'SomeData' not in get_names(walker())
...
def test_kwargs():
  ...
  assert get_names(PT.NodesWalkers(node, predicates, depth=1)()) == [] #First predicate is not found
  root = PT.get_node_from_name(node, "RefStateFamilyBCDataSet")
  assert get_names(PT.NodesWalkers(root, predicates, depth=1)()) == ['Density', 'MomentumX'] 
  ...
  assert get_names(PT.NodesWalkers(node, predicates)()) == ['Density', 'MomentumX'] 
