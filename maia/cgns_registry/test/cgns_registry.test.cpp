...
// ---------------------------------------------------------------------------------
MPI_TEST_CASE("fixtured test on 2 procs",2) {
  int n_rank_l, i_rank_l;
  MPI_Comm_size(test_comm, &n_rank_l);
  MPI_Comm_rank(test_comm, &i_rank_l);

  SUBCASE("[Subcase] inside fixture ") {

    CHECK(test_nb_procs == 2);
    // CHECK(test_nb_procs == 0);
    // DOCTEST_MPI_CHECK(0, n_rank_l == 2);
    // DOCTEST_MPI_CHECK(0, i_rank_l == 0);

    // DOCTEST_MPI_CHECK(1, n_rank_l == 2);
    // DOCTEST_MPI_CHECK(1, i_rank_l == 0);
...
// ---------------------------------------------------------------------------------
MPI_TEST_CASE("fixtured test on 3 procs",3) {
  int n_rank_l, i_rank_l;
  MPI_Comm_size(test_comm, &n_rank_l);
  MPI_Comm_rank(test_comm, &i_rank_l);
  ...
  SUBCASE("[Subcase] inside fixture ") {
    ...
    CHECK(test_nb_procs == 3);
    // CHECK(test_nb_procs == 0);
    DOCTEST_MPI_CHECK(0, n_rank_l == 3);
    DOCTEST_MPI_CHECK(0, i_rank_l == 0);

    DOCTEST_MPI_CHECK(1, n_rank_l == 3);
    DOCTEST_MPI_CHECK(1, i_rank_l == 1);
    // DOCTEST_MPI_CHECK(2, i_rank_l == 1);
