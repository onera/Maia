...
create_base_two_ranks(int mpi_rank, factory F) -> tree {
  ...
  tree b = F.new_CGNSBase("Base",3,3);
...
*/
  ...
  if (mpi_rank == 0) {
  // Zone0
    tree z0 = F.new_UnstructuredZone<I4>("Zone0");

    auto pld00_data = make_cgns_vector( {1,2,3} , F.alloc() );
    tree pld00 = F.new_PointList("PointListDonor",std_e::make_span(pld00_data));
    tree gc00 = F.new_GridConnectivity("Join0","Zone0","FaceCenter","Abutting1to1");
    emplace_child(gc00,std::move(pld00));

    auto pld01_data = make_cgns_vector( {11,12,13,14} , F.alloc() );
    tree pld01 = F.new_PointList("PointListDonor",std_e::make_span(pld01_data));
    tree gc01 = F.new_GridConnectivity("Join1","Zone1","FaceCenter","Abutting1to1");
    emplace_child(gc01,std::move(pld01));

    tree zone_gc0 = F.new_ZoneGridConnectivity();
  ...
  // Zone3
    tree z3 = F.new_UnstructuredZone<I4>("Zone3");

    auto pld31a_data = make_cgns_vector( {15} , F.alloc() );
    tree pld31a = F.new_PointList("PointListDonor",std_e::make_span(pld31a_data));
    tree gc31a = F.new_GridConnectivity("Join2","Zone1","Vertex","Abutting1to1");
    emplace_child(gc31a,std::move(pld31a));

    auto pld31b_data = make_cgns_vector( {16,17} , F.alloc() );
    tree pld31b = F.new_PointList("PointListDonor",std_e::make_span(pld31b_data));
    tree gc31b = F.new_GridConnectivity("Join3","Zone1","Vertex","Abutting1to1");
    emplace_child(gc31b,std::move(pld31b));

    tree zone_gc3 = F.new_ZoneGridConnectivity();
  ...
  // Zone1
    tree z1 = F.new_UnstructuredZone<I4>("Zone1");

    auto pld11_data = make_cgns_vector( {101,102,103,104} , F.alloc() );
    tree pld11 = F.new_PointList("PointListDonor",std_e::make_span(pld11_data));
    tree gc11 = F.new_GridConnectivity("Join4","Zone1","CellCenter","Abutting1to1");
    emplace_child(gc11,std::move(pld11));

    auto pld10_data = make_cgns_vector( {111,112} , F.alloc() );
    tree pld10 = F.new_PointList("PointListDonor",std_e::make_span(pld10_data));
    tree gc10 = F.new_GridConnectivity("Join5","Zone0","Vertex","Abutting1to1");
    emplace_child(gc10,std::move(pld10));

    tree zone_gc1 = F.new_ZoneGridConnectivity();
  ...
  // Zone2
    tree z2 = F.new_UnstructuredZone<I4>("Zone2");

    auto pld21_data = make_cgns_vector( {136,137} , F.alloc() );
    tree pld21 = F.new_PointList("PointListDonor",std_e::make_span(pld21_data));
    tree gc21 = F.new_GridConnectivity("Join6","Zone3","CellCenter","Abutting1to1");
    emplace_child(gc21,std::move(pld21));

    tree zone_gc2 = F.new_ZoneGridConnectivity();
