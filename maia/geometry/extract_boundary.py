...
class ExtractSurfFromBC:
  ...
  def compute(self, families):
    ...
    for zone_node in zones:
      ...
      LOG.info(f"merge_pl_idx_zone.shape[0] [vertex] = {merge_pl_idx_zone.shape[0]}")
      LOG.info(f"merge_pl_zone.size [vertex] = {merge_pl_zone.size}")
      if merge_pl_zone.size > 0:
      ...
      LOG.info(f"merge_pl_idx_zone.shape[0] [face] = {merge_pl_idx_zone.shape[0]}")
      LOG.info(f"merge_pl_zone.size [face] = {merge_pl_zone.size}")
      if merge_pl_zone.size > 0:
