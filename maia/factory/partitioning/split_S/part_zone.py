...
def collect_S_bnd_per_dir(zone):
  ...
  # Remove extra dims
  base_bound = {key:val for key, val in base_bound.items() if val != []}
  return base_bound
...
def create_bcs(d_zone, p_zone, p_zone_offset):
  ...
  for geo_bnd, dist_bnds in collect_S_bnd_per_dir(d_zone).items():
    ...
    if is_old_bc:
      idx_dim = range_part_bc_g.shape[0]
      ...
      for dist_bc in dist_bnds:
        ...
        if inter is not None:
          sub_pr = np.empty((idx_dim,2), dtype=np.int32)
          sub_pr[dirs,:] = inter
...
def create_zone_gnums(cell_window, dist_zone_cell_size, dtype=pdm_dtype):
  ...
  # Faces
  if idx_dim == 3:
    face_lntogn = compute_face_gnum(dist_zone_cell_size, cell_window, dtype=pdm_dtype)
