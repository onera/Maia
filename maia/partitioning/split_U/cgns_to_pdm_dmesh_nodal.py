...
from .cgns_to_pdm_dmesh import concatenate_point_list
from maia.utils import py_utils

from Pypdm.Pypdm import DistributedMeshNodal

# --------------------------------------------------------------------------
def concatenate_bc(dist_zone):
  """
  ...
  """
  point_lists = []
  for bc in py_utils.getNodesFromTypePath(dist_zone, 'ZoneBC_t/BC_t'):
    if I.getNodeFromName1(bc, 'PointList') is not None:
      point_lists.append(I.getNodeFromName1(bc, 'PointList')[1])
    elif I.getNodeFromName1(bc, 'PointRange') is not None:
      pr = I.getNodeFromName1(bc, 'PointRange')[1]
      distrib_n = I.getNodeFromPath(bc, ':CGNS#Distribution/Distribution')
...
def cgns_dist_zone_to_pdm_dmesh_nodal(dist_zone,comm):
  ...
  # Boundaries
  delmt_bound_idx, delmt_bound =  concatenate_bc(dist_zone)
