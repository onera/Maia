...
def belongs_to_family(n, target_family, allow_additional=False):
  ...
  family_name_n = requestNodeFromPredicate(n, lambda m: match_cgk_label(m, CGL.FamilyName_t), depth=1)
  if family_name_n and I.getValue(family_name_n) == target_family:
    return True
  if allow_additional:
    for additional_family_n in iterNodesFromPredicate(n, lambda m: match_cgk_label(m, CGL.AdditionalFamilyName_t), depth=1):
