...
def is_same_value_type(n0: TreeNode, n1: TreeNode):
...
def is_same_value(n0: TreeNode, n1: TreeNode, abs_tol=0):
  if not is_same_value_type(n0,n1):
    return False
  if n0[1] is None:
    return True
...
def is_same_node(node1, node2, abs_tol=0):
  """
  ...
  """
  return is_same_name(node1, node2) and is_same_label(node1, node2) and is_same_value(node1, node2, abs_tol) 

def is_same_tree(node1, node2, abs_tol=0):
  """
  ...
  """
  if not (is_same_node(node1, node2, abs_tol) and len(I.getChildren(node1)) == len(I.getChildren(node2)) ):
    return False
  for c1, c2 in zip(sorted(node1[2]), sorted(node2[2])):
    if not is_same_tree(c1, c2, abs_tol):
      return False
  return True
