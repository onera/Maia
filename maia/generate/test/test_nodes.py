...
def test_concatenate_subset_nodes(sub_comm):
  ...
  subset_nodes = [DN.distribute_pl_node(node, sub_comm) for node in subset_nodes_f]

  expected_distri = DF.uniform_distribution(4+8, sub_comm)
  ...
  assert sids.GridLocation(node) == 'FaceCenter'
  assert (IE.getDistribution(node, 'Index')[1] == expected_distri).all()
