...
def match_jn_from_ordinals(dist_tree):
  ...
  for base,zone,zgc,gc in IE.getNodesWithParentsFromTypeMatching(dist_tree, gc_t_path):
...
def disttree_from_parttree(part_tree, comm):
  ...
  for dist_base, dist_zone in IE.getNodesWithParentsFromTypeMatching(dist_tree, 'CGNSBase_t/Zone_t'):
    ...
    # > Index exchange
    for d_zbc, d_bc in IE.getNodesWithParentsFromTypeMatching(dist_zone, bc_t_path):
      IPTB.part_pl_to_dist_pl(dist_zone, part_zones, I.getName(d_zbc) + '/' + I.getName(d_bc), comm)
    for d_zgc, d_gc in IE.getNodesWithParentsFromTypeMatching(dist_zone, gc_t_path):
